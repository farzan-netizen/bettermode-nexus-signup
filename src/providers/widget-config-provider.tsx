...
interface WidgetConfigContextType {
  ...
  updateAnnouncementBannerConfig: (
    config: Partial<AnnouncementBannerConfig>,
  ) => void
  ...
  addSpaceWidget: (
    widget: Omit<SpaceWidget, 'enabled'>,
    containerId?: string,
  ) => void
...
export const WidgetConfigProvider: React.FC<WidgetConfigProviderProps> = ({
  ...
  const [eventsListConfig, setEventsListConfig] = useState<EventsListConfig>(
    defaultEventsListConfig,
  )
  const [spaceHeaderConfig, setSpaceHeaderConfig] = useState<SpaceHeaderConfig>(
    defaultSpaceHeaderConfig,
  )
  const [announcementBannerConfig, setAnnouncementBannerConfig] =
    useState<AnnouncementBannerConfig>(defaultAnnouncementBannerConfig)
  const [heroBannerConfig, setHeroBannerConfig] = useState<HeroBannerConfig>(
    defaultHeroBannerConfig,
  )
  const [membersConfig, setMembersConfig] =
    useState<MembersConfig>(defaultMembersConfig)
  ...
  const [htmlScriptConfig, setHtmlScriptConfig] = useState<HtmlScriptConfig>(
    defaultHtmlScriptConfig,
  )
  const [richTextConfig, setRichTextConfig] = useState<RichTextConfig>(
    defaultRichTextConfig,
  )
  const [discussionsListConfig, setDiscussionsListConfig] =
    useState<DiscussionsListConfig>(defaultDiscussionsListConfig)
  const [knowledgesListConfig, setKnowledgesListConfig] =
    useState<KnowledgesListConfig>(defaultKnowledgesListConfig)
  const [wishlistsListConfig, setWishlistsListConfig] =
    useState<WishlistsListConfig>(defaultWishlistsListConfig)
  const [questionsListConfig, setQuestionsListConfig] =
    useState<QuestionsListConfig>(defaultQuestionsListConfig)
  ...
  const [spaceWidgetStates, setSpaceWidgetStates] = useState<SpaceWidgetStates>(
    () => {
      ...
      const isGrowthDiscussion =
        currentPath.includes('/site/spaces/growth/discussion') ||
        currentPath.includes('/design/spaces/discussions')
      const isGrowthQuestion =
        currentPath.includes('/site/spaces/growth/question') ||
        currentPath.includes('/design/spaces/questions')
      const isGrowthWishlist =
        currentPath.includes('/site/spaces/growth/wishlist') ||
        currentPath.includes('/design/spaces/wishlist')
      const isGrowthEvents =
        currentPath.includes('/site/spaces/growth/events') ||
        currentPath.includes('/design/spaces/events')
      const isDesignHome =
        currentPath === '/admin4/design' ||
        currentPath.includes('/design/page-customizer')
  ...
  const [sidebarWidgetStates, setSidebarWidgetStates] =
    useState<SidebarWidgetStates>({
      quickActions: true,
  ...
  const updateAnnouncementBannerConfig = (
    config: Partial<AnnouncementBannerConfig>,
  ) => {
  ...
  const updateDiscussionsListConfig = (
    config: Partial<DiscussionsListConfig>,
  ) => {
    setDiscussionsListConfig(prevConfig => ({ ...prevConfig, ...config }))
  }

  const updateKnowledgesListConfig = (
    config: Partial<KnowledgesListConfig>,
  ) => {
  ...
  const removeSpaceWidget = (widgetId: string) => {
    setSpaceWidgetStates(prevStates => ({
      ...prevStates,
      dynamicWidgets: prevStates.dynamicWidgets.filter(
  ...
  const duplicateSpaceWidget = (widgetId: string) => {
    const widgetToDuplicate = spaceWidgetStates.dynamicWidgets.find(
      widget => widget.id === widgetId,
    )
  ...
  const duplicateSidebarWidget = (widgetId: string) => {
    const widgetToDuplicate = sidebarWidgetStates.dynamicWidgets.find(
      widget => widget.id === widgetId,
    )
    if (widgetToDuplicate) {
      const newWidget: SidebarWidget = {
