...
export const Step4Industry = ({
  ...
  const [originalPreselection, setOriginalPreselection] = useState<
    string | null
  >(null)
  ...
  const findMatchingIndustry = (brandIndustryName: string): string | null => {
    ...
    const directMatch = industries.find(
      industry =>
      ...
      topSlot={
        <div className="">
          <p className="text-lg text-tertiary">
            Nice to meet you{' '}
            <span className="font-bold">{formData.firstName}</span>!
          </p>
        </div>
      }
    >
          ...
          brandData.company.industries.length > 0 && (
            <div className="text-left">
              <p className="text-sm text-tertiary">
                ...
                {industries.find(i => i.id === formData.industry)?.name}". Feel
                free to select something else if this doesn't look right.
              </p>
            </div>
          )}
