  ...
  switch (step) {
    ...
    case 2:
      ...
      if (formData.verificationCode === '111111')
        newErrors.verificationCode = 'Incorrect code entered'
      if (formData.verificationCode === '222222')
        newErrors.verificationCode = 'Code has been expired, tap to resend'
      break
    case 3:
...
export const getRecommendedPlan = (
  ...
  const hasAdvancedIntegrations = formData.currentTools.some(tool =>
...
const formatUserCount = (count: string): string => {
  if (count.startsWith('under-')) {
    ...
    return count
      ...
      .replace('25000', '25,000')
      .replace('50000', '50,000')
  }
}
...
const getBenefitsByPlan = (plan: string): string[] => {
  ...
  return (
    benefits[plan.toLowerCase() as keyof typeof benefits] || benefits.starter
  )
}
