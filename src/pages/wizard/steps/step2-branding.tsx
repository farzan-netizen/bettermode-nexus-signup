...
import { StepContainer } from '../../step-container.tsx'
...
export const Step2Branding = ({
    ...
    if (savedBrandData) {
      try {
        ...
        if (brandData?.colors && Array.isArray(brandData.colors)) {
          const colorHexes = brandData.colors
            .map((color: any) => {
        ...
        if (brandData.logos && Array.isArray(brandData.logos)) {
          ...
          brandData.logos.forEach((logoGroup: any) => {
            if (logoGroup.formats && Array.isArray(logoGroup.formats)) {
              const svgFormat = logoGroup.formats.find(
                (f: any) => f.format === 'svg',
              )
              const pngFormat = logoGroup.formats.find(
                (f: any) => f.format === 'png',
              )
      ...
      if (brandData) {
        ...
        if (brandData.logos && Array.isArray(brandData.logos)) {
          ...
          if (logos && Array.isArray(logos) && logos.length > 0) {
            ...
            if (
              firstLogoGroup &&
              firstLogoGroup.formats &&
              Array.isArray(firstLogoGroup.formats) &&
              firstLogoGroup.formats.length > 0
            ) {
  ...
  return (
    <StepContainer title="Branding" description={getStepDescription()}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-primary border border-secondary rounded-lg p-4">
            ...
            {(formData.isManualBranding || suggestedColors.length === 0) &&
              !isLoading && (
                <div>
        ...
        <div className="flex justify-end items-center gap-6 pt-4">
          ...
          {/* Continue Button */}
          <Button iconTrailing={ArrowRight} onClick={onNext} size="sm">
            Continue
          </Button>
        </div>
      </div>
